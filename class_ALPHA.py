...
with open('{}/data/clf_ALPHA.pickle'.format(mod_path), mode='rb') as file:
    clf_ALPHA = pickle.load(file)#预训练好的模型
...
def run_clfALPHA(dic, text, verbose=True, user_abbrevs={}):
    ...
    for (ind, (nsw, tag)) in dic.items():
        if verbose:
            ...
            sys.stdout.flush()
        if romanNumeralPattern.match(nsw) and gen_frame((ind, (nsw, tag)), text)[1].lower() in names_lower:##此句进行罗马数字匹配
            out.update({ind: (nsw, 'NUMB', 'NORD')})#罗马数字对应的是NORD
        elif nsw in user_abbrevs:#可以定制缩写单词，需要给予参数
            out.update({ind: (nsw, 'ALPHA', 'EXPN')})
        else:
            pred_int = int(clf.predict(gen_featuresetsALPHA({ind: (nsw, tag)}, text)))#用一个分类树来进行分类了,很多类型是错到这里了
